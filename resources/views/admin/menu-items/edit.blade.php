...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow">
            ...
            <form action="{{ route('admin.menu-items.update', $menuItem) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text"
                                   name="title_id"
                                   id="title_id"
                                   value="{{ old('title_id', $menuItem->title_id) }}"
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="title_en"
                                   id="title_en"
                                   value="{{ old('title_en', $menuItem->title_en) }}"
                    ...
                    <div>
                        ...
                        <select name="location" 
                            ...
                            @foreach($locations as $key => $label)
                                <option value="{{ $key }}" {{ old('location', $menuItem->location) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="slug_id" class="block text-sm font-medium text-gray-700 mb-2">
                                Slug (ID)
                            </label>
                            <input type="text"
                                   name="slug_id"
                                   id="slug_id"
                                   value="{{ old('slug_id', $menuItem->slug_id) }}"
                                   placeholder="contoh: tentang-kami"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('slug_id') border-red-500 @enderror">
                            @error('slug_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="slug_en" class="block text-sm font-medium text-gray-700 mb-2">
                                Slug (EN)
                            </label>
                            <input type="text"
                                   name="slug_en"
                                   id="slug_en"
                                   value="{{ old('slug_en', $menuItem->slug_en) }}"
                                   placeholder="example: about-us"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('slug_en') border-red-500 @enderror">
                            @error('slug_en')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" 
                               ...
                               id="open_new_tab" 
                               value="1"
                               {{ old('open_new_tab', $menuItem->open_new_tab) ? 'checked' : '' }}
                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
