...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Kelola testimoni yang disubmit oleh user dari halaman beranda</p>
        </div>
    </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="text-sm font-medium text-gray-900 truncate">{{ $testimonial->client_name }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    @for($i = 1; $i <= 5; $i++)
                                        <i
                                            class="fas fa-star {{ $i <= $testimonial->rating ? 'text-yellow-400' : 'text-gray-300' }} text-sm"></i>
                                    @endfor
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @if(isset($testimonial->is_survey) && $testimonial->is_survey === true)
                                        ...
                                        <form action="{{ route('admin.surveys.toggle-visibility', $testimonial->survey_id) }}"
                                            method="POST" class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="text-{{ $testimonial->show_on_home ? 'green' : 'gray' }}-600 hover:text-{{ $testimonial->show_on_home ? 'green' : 'gray' }}-900"
                                                title="{{ ($testimonial->show_on_home ?? false) ? 'Sembunyikan di beranda' : 'Tampilkan di beranda' }}">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                        </form>

                                        <form action="{{ route('admin.surveys.destroy', $testimonial->survey_id) }}" method="POST"
                                            class="inline"
                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus survey ini?')">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        ...
                                        <form action="{{ route('admin.testimonials.toggle-approval', $testimonial) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="text-{{ $testimonial->is_approved ? 'yellow' : 'green' }}-600 hover:text-{{ $testimonial->is_approved ? 'yellow' : 'green' }}-900"
                                                title="{{ $testimonial->is_approved ? 'Batalkan Persetujuan' : 'Setujui' }}">
