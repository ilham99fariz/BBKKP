                        ...
                        <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">
                                            ...
                                            @if($item->views)
                                                ...
                                                <i class="fas fa-eye mr-1"></i>{{ number_format($item->views) }} views
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-600 max-w-md">
                                    <div class="line-clamp-2">{{ Str::limit($item->excerpt ?? strip_tags($item->content), 120) }}</div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($item->published_at)
                                    <div class="text-sm">{{ $item->published_at->format('d M Y') }}</div>
                                    <div class="text-xs text-gray-500">{{ $item->published_at->format('H:i') }}</div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <form action="{{ route('admin.news.toggle-publish', $item) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" 
                                                class="text-{{ $item->is_published ? 'yellow' : 'green' }}-600 hover:text-{{ $item->is_published ? 'yellow' : 'green' }}-900 transition-colors"
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $news->where('is_published', true)->count() }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $news->where('is_published', false)->count() }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $news->whereNotNull('position')->count() }}</p>
                </div>
            </div>
        </div>
    </div>
