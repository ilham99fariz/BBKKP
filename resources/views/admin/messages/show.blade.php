...
<div class="space-y-6">
        ...
        <div class="flex-1">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="mt-1 text-sm text-gray-500">Dikirim oleh <strong class="text-gray-800">{{ $message->name }}</strong> — {{ $message->created_at->format('d M Y H:i') }}</div>
                </div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    @endif
                    <span class="inline-flex items-center px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-sm">{{ $message->purpose ? ucfirst(str_replace('_', ' ', $message->purpose)) : 'Umum' }}</span>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="text-sm text-gray-600">
                        <p><span class="font-medium text-gray-800">Email:</span> <a href="mailto:{{ $message->email }}" class="text-blue-600">{{ $message->email }}</a></p>
                        <p class="mt-2"><span class="font-medium text-gray-800">Telepon:</span> {{ $message->phone ?? '-' }}</p>
                    </div>

                    <div class="text-sm text-gray-600">
                        <p><span class="font-medium text-gray-800">Perusahaan:</span> {{ $message->company ?? '-' }}</p>
                        <p class="mt-2"><span class="font-medium text-gray-800">Diterima:</span> {{ $message->created_at->diffForHumans() }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6 flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.messages.index') }}" class="px-4 py-2 border rounded-md text-sm">Kembali ke Daftar</a>
                </div>
            </div>
        </div>
        ...
        <aside class="bg-white rounded-xl shadow p-6">
            ...
            @endif
            @if(session('error'))
                <div class="bg-red-50 border border-red-200 text-red-800 px-3 py-2 rounded mb-3">{{ session('error') }}</div>
            @endif
            ...
            <form action="{{ route('admin.messages.reply', $message) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="email" value="{{ $message->email }}" readonly class="w-full px-3 py-2 border rounded bg-gray-50 text-sm" />
                </div>

                <div class="mb-3">
                    <label for="reply_message" class="block text-sm font-medium text-gray-700 mb-1">Pesan Balasan</label>
                    <textarea id="reply_message" name="reply_message" rows="6" required class="w-full px-3 py-2 border rounded text-sm @if($errors->has('reply_message')) border-red-500 @endif">{{ old('reply_message') }}</textarea>
                    @if($errors->has('reply_message'))
                        <p class="text-red-500 text-sm mt-1">{{ $errors->first('reply_message') }}</p>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="mailto:{{ $message->email }}" class="inline-flex items-center px-4 py-2 border rounded-md text-sm text-white bg-green-700">Buka di Email</a>
                </div>
